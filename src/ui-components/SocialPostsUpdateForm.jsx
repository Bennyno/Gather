...
export default function SocialPostsUpdateForm(props) {
  const {
    id: idProp,
    socialPosts: socialPostsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = socialPostsRecord
      ? { ...initialValues, ...socialPostsRecord }
      : initialValues;
    ...
    setSharesCount(cleanValues.sharesCount);
    setLikesCount(cleanValues.likesCount);
    setCrosspostCount(cleanValues.crosspostCount);
    setLikedBy(cleanValues.likedBy ?? []);
    setCurrentLikedByValue("");
    setErrors({});
  };
  const [socialPostsRecord, setSocialPostsRecord] =
    React.useState(socialPostsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(SocialPosts, idProp)
        : socialPostsModelProp;
      setSocialPostsRecord(record);
    };
    queryData();
  }, [idProp, socialPostsModelProp]);
  React.useEffect(resetStateValues, [socialPostsRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            SocialPosts.copyOf(socialPostsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SocialPostsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Message"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || socialPostsModelProp)}
